...
#define WORKER_ALL_RX_EVENTS                                                   \
  (WorkerEvtStop | WorkerEvtRxDone | WorkerEvtCfgChange |                      \
   WorkerEvtLineCfgSet | WorkerEvtCtrlLineSet | WorkerEvtCdcTxComplete)
...
static void vcp_on_line_config(void *context,
...
static void usb_uart_on_irq_rx_dma_cb(FuriHalSerialHandle *handle,
                                      FuriHalSerialRxEvent ev, size_t size,
                                      void *context) {
  ...
  if (ev & (FuriHalSerialRxEventData | FuriHalSerialRxEventIdle)) {
    ...
    furi_thread_flags_set(furi_thread_get_id(usb_uart->thread),
                          WorkerEvtRxDone);
  }
}
...
static void usb_uart_serial_init(UsbUartBridge *usb_uart, uint8_t uart_ch) {
  ...
  furi_hal_serial_dma_rx_start(usb_uart->serial_handle,
                               usb_uart_on_irq_rx_dma_cb, usb_uart, false);
}
...
static void usb_uart_update_ctrl_lines(UsbUartBridge *usb_uart) {
  if (usb_uart->cfg.flow_pins != 0) {
    ...
    furi_hal_gpio_write(flow_pins[usb_uart->cfg.flow_pins - 1][0],
                        !(state & USB_CDC_BIT_RTS));
    furi_hal_gpio_write(flow_pins[usb_uart->cfg.flow_pins - 1][1],
                        !(state & USB_CDC_BIT_DTR));
  }
}
...
static int32_t usb_uart_worker(void *context) {
  ...
  usb_uart->tx_thread = furi_thread_alloc_ex("UsbUartTxWorker", 512,
                                             usb_uart_tx_thread, usb_uart);
  ...
  furi_thread_flags_set(furi_thread_get_id(usb_uart->tx_thread),
                        WorkerEvtCdcRx);
    ...
    if (events & WorkerEvtCfgChange) {
      ...
      if (usb_uart->cfg.flow_pins != usb_uart->cfg_new.flow_pins) {
        ...
        if (usb_uart->cfg_new.flow_pins != 0) {
          furi_assert((size_t)(usb_uart->cfg_new.flow_pins - 1) <
                      COUNT_OF(flow_pins));
          furi_hal_gpio_init_simple(
              flow_pins[usb_uart->cfg_new.flow_pins - 1][0],
              GpioModeOutputPushPull);
          furi_hal_gpio_init_simple(
              flow_pins[usb_uart->cfg_new.flow_pins - 1][1],
              GpioModeOutputPushPull);
        }
      ...
      if (usb_uart->cfg.software_de_re != usb_uart->cfg_new.software_de_re) {
        ...
        if (usb_uart->cfg.software_de_re != 0) {
          ...
          furi_hal_gpio_init(USB_USART_DE_RE_PIN, GpioModeOutputPushPull,
...
static int32_t usb_uart_tx_thread(void *context) {
  ...
  while (1) {
    uint32_t events = furi_thread_flags_wait(WORKER_ALL_TX_EVENTS,
                                             FuriFlagWaitAny, FuriWaitForever);
    furi_check(!(events & FuriFlagError));
    ...
    if (events & WorkerEvtCdcRx) {
      furi_check(furi_mutex_acquire(usb_uart->usb_mutex, FuriWaitForever) ==
                 FuriStatusOk);
      size_t len =
          furi_hal_cdc_receive(usb_uart->cfg.vcp_ch, data, USB_CDC_PKT_LEN);
      furi_check(furi_mutex_release(usb_uart->usb_mutex) == FuriStatusOk);
...
UsbUartBridge *usb_uart_enable(UsbUartConfig *cfg) {
  ...
  usb_uart->thread =
      furi_thread_alloc_ex("UsbUartWorker", 1024, usb_uart_worker, usb_uart);
...
// void usb_uart_send(UsbUartBridge* usb_uart, void* data, size_t len) {
//
// }
